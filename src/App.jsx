...
const ADMIN_EMAIL = "waldraffnico@eclipso.de";
...
export default function HumanRightsHub() {
  ...
  const loadPosts = async (topicId) => {
    ...
    const { data } = await supabase.from("posts").select("*").eq("topic_id", topicId).order("created_at", { ascending: true });
    setPosts(data || []);
    if (user) {
  ...
  const incrementTopicViews = async (topicId) => {
    ...
    } catch (e) { console.log("views error", e); }
  ...
  const handleAuth = async () => {
    ...
    if (authMode === "register") {
      ...
      if (error) {
        ...
        setAuthError("Bitte bestätige deine E-Mail-Adresse. Wir haben dir eine E-Mail geschickt!");
      }
    } else {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "16px", height: "calc(100vh - 180px)" }}>
      <div style={S.chatHeader}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.35)" }}>{selectedTopic?.category} · 👁 {topicViews[selectedTopic?.id] || 0} Aufrufe</div>
        </div>
      </div>
      <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: "10px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "8px", padding: "14px" }}>
        ...
        {!postsLoading && posts.length === 0 && (
          <div style={{ textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: "13px", padding: "20px" }}>Noch keine Beiträge. Sei die erste Person, die schreibt!</div>
        )}
        {posts.map(post => (
          <div key={post.id} style={{ background: "rgba(255,255,255,0.04)", borderRadius: "10px", padding: "12px 14px", border: "1px solid rgba(255,255,255,0.07)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
              ...
              <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)" }}>{new Date(post.created_at).toLocaleDateString("de-DE", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" })}</span>
            </div>
      ...
      {user ? (
        <div style={{ display: "flex", gap: "8px" }}>
          <input style={S.input} value={newPostText} onChange={e => setNewPostText(e.target.value)}
            onKeyDown={e => e.key === "Enter" && submitPost()} placeholder="Dein Beitrag zur Diskussion…" />
          <button style={S.sendBtn(postLoading || !newPostText.trim())} onClick={submitPost} disabled={postLoading || !newPostText.trim()}>Posten</button>
        </div>
      ) : (
        <div style={S.loginPrompt} onClick={() => setShowAuth(true)}>🔐 Anmelden, um an der Diskussion teilzunehmen</div>
      )}
      <div style={{ borderTop: "1px solid rgba(200,150,62,0.15)", paddingTop: "12px" }}>
        ...
        <div style={{ maxHeight: "180px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "8px", marginBottom: "8px" }}>
          ...
          {aiLoading && <div style={msgBubbleStyle("ai")}><div style={S.msgName}>MR-Assistent</div><div style={{ color: "rgba(255,255,255,0.4)", fontSize: "13px" }}>schreibt...</div></div>}
          <div ref={messagesEndRef} />
        </div>
        <div style={{ display: "flex", gap: "8px" }}>
          <input style={S.input} value={inputMsg} onChange={e => setInputMsg(e.target.value)}
            onKeyDown={e => e.key === "Enter" && sendForumMessage()} placeholder="Frage an den KI-Assistenten..." />
  ...
  const renderRightsChat = () => (
    <div style={S.chatContainer}>
      ...
      <div style={S.chatMessages}>
        ...
        {chatLoading && <div style={msgBubbleStyle("ai")}><div style={S.msgName}>MR-Experte</div><div style={{ color: "rgba(255,255,255,0.4)", fontSize: "13px" }}>analysiert...</div></div>}
        <div ref={chatEndRef} />
      </div>
      <div style={S.chatInput}>
        <input style={S.input} value={chatInput} onChange={e => setChatInput(e.target.value)}
          onKeyDown={e => e.key === "Enter" && sendChatMessage()} placeholder="Frage stellen..." />
  ...
  const renderActions = () => {
    ...
    if (activeAction === "petition") return (
      <div>
        ...
        {[
          ...
          { title: "Flüchtlingsschutz stärken", signatures: "67.890", url: "https://www.change.org/search?q=fluechtlingsschutz" },
        ].map(p => (
          <div key={p.title} style={{ padding: "16px 18px", borderRadius: "12px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(200,150,62,0.2)", marginBottom: "10px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <a href={p.url} target="_blank" rel="noreferrer" style={{ padding: "8px 16px", borderRadius: "8px", background: "#C8963E", color: "#0D1B2A", fontWeight: "700", fontSize: "13px", textDecoration: "none" }}>Unterschreiben →</a>
          </div>
        ))}
      </div>
    );

    if (activeAction === "brief") return (
      <div>
        <button style={S.chatBack} onClick={() => setActiveAction(null)}>← Zurück</button>
        <h2 style={{ fontSize: "22px", fontWeight: "800", margin: "16px 0 8px" }}>📧 Brief schreiben</h2>
        <textarea style={{ width: "100%", minHeight: "320px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(200,150,62,0.3)", borderRadius: "12px", padding: "16px", color: "#E8E8E8", fontSize: "13px", lineHeight: "1.7", outline: "none", resize: "vertical", boxSizing: "border-box" }}
          value={briefText || briefVorlage} onChange={e => setBriefText(e.target.value)} />
        <div style={{ display: "flex", gap: "10px", marginTop: "12px" }}>
          <button onClick={() => navigator.clipboard.writeText(briefText || briefVorlage)}
            style={{ flex: 1, padding: "12px", borderRadius: "8px", border: "1px solid rgba(200,150,62,0.3)", background: "rgba(200,150,62,0.1)", color: "#C8963E", fontWeight: "700", cursor: "pointer", fontSize: "13px" }}>📋 Kopieren</button>
          <a href={`mailto:?subject=Menschenrechte&body=${encodeURIComponent(briefText || briefVorlage)}`}
            style={{ flex: 1, padding: "12px", borderRadius: "8px", border: "none", background: "#C8963E", color: "#0D1B2A", fontWeight: "700", fontSize: "13px", textDecoration: "none", textAlign: "center" }}>📧 Per E-Mail senden</a>
        </div>
      </div>
    );

    if (activeAction === "awareness") return (
      <div>
        <button style={S.chatBack} onClick={() => setActiveAction(null)}>← Zurück</button>
        <h2 style={{ fontSize: "22px", fontWeight: "800", margin: "16px 0 8px" }}>📣 Awareness teilen</h2>
        {[
          { platform: "Instagram", icon: "📸", tip: "Teile Infografiken in deiner Story. Nutze #HumanRights #Menschenrechte #StandUp" },
          { platform: "Twitter / X", icon: "🐦", tip: "Tweete täglich einen Menschenrechtsartikel. Tagge @amnesty oder @hrw für mehr Reichweite." },
          { platform: "TikTok", icon: "🎵", tip: "Erkläre in 60 Sekunden einen Artikel der UN-Charta – kurze Videos erreichen Millionen." },
          { platform: "WhatsApp", icon: "💬", tip: `Teile diesen Link: ${window.location.href}` },
        ].map(item => (
    ...
    if (activeAction === "orgs") return (
      <div>
        <button style={S.chatBack} onClick={() => setActiveAction(null)}>← Zurück</button>
        <h2 style={{ fontSize: "22px", fontWeight: "800", margin: "16px 0 8px" }}>🤝 Organisationen unterstützen</h2>
        {[
          { name: "Amnesty International", desc: "Weltweit führende Menschenrechtsorganisation", url: "https://www.amnesty.de", color: "#C8963E" },
          { name: "Human Rights Watch", desc: "Dokumentiert Menschenrechtsverletzungen weltweit", url: "https://www.hrw.org/de", color: "#4A90D9" },
          { name: "UNHCR", desc: "UN-Flüchtlingshilfswerk – schützt Flüchtlinge weltweit", url: "https://www.unhcr.org/de", color: "#6BAE75" },
          { name: "Reporter ohne Grenzen", desc: "Kämpft für Pressefreiheit weltweit", url: "https://www.reporter-ohne-grenzen.de", color: "#9B6DB5" },
          { name: "UNICEF Deutschland", desc: "Schützt Kinderrechte weltweit", url: "https://www.unicef.de", color: "#E05C5C" },
        ].map(org => (
          <div key={org.name} style={{ padding: "16px 18px", borderRadius: "12px", background: "rgba(255,255,255,0.03)", border: `1px solid ${org.color}25`, marginBottom: "10px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
            <div>
              ...
              <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.45)" }}>{org.desc}</div>
            </div>
            <a href={org.url} target="_blank" rel="noreferrer" style={{ padding: "8px 14px", borderRadius: "8px", background: `${org.color}20`, color: org.color, fontWeight: "700", fontSize: "12px", textDecoration: "none", whiteSpace: "nowrap", border: `1px solid ${org.color}30` }}>Besuchen →</a>
          </div>
        ))}
      </div>
    );
    ...
    return (
      <div>
        <div style={{ marginBottom: "20px" }}>
          ...
          <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.45)" }}>Jede Stimme zählt.</p>
        </div>
        <div style={S.actionGrid}>
          {[
            { key: "petition", icon: "✍️", title: "Petition unterzeichnen", desc: "Aktuelle Petitionen auf Change.org", count: "Zu Change.org", color: "#C8963E" },
            { key: "brief", icon: "📧", title: "Brief schreiben", desc: "Vorlage für Politiker", count: "Vorlage verfügbar", color: "#4A90D9" },
            { key: "awareness", icon: "📣", title: "Awareness teilen", desc: "Anleitung für soziale Medien", count: "4 Plattformen", color: "#6BAE75" },
            { key: "orgs", icon: "🤝", title: "Organisationen unterstützen", desc: "Amnesty, HRW & mehr", count: "5 Partner", color: "#9B6DB5" },
          ].map(action => (
            <div key={action.key} style={{ ...S.actionCard(action.color), padding: "24px", cursor: "pointer" }}
              ...
              <div style={{ marginTop: "14px", padding: "10px", background: `${action.color}15`, borderRadius: "8px", border: `1px solid ${action.color}25`, textAlign: "center", fontSize: "13px", fontWeight: "700", color: action.color }}>Jetzt →</div>
            </div>
          ))}
        </div>
      </div>
  ...
  const renderBlog = () => {
    ...
    if (selectedPost) return (
      <div>
        ...
        {selectedPost.image_url && <img src={selectedPost.image_url} style={{ width: "100%", borderRadius: "12px", marginTop: "16px", marginBottom: "20px", maxHeight: "300px", objectFit: "cover" }} />}
        <h2 style={{ fontSize: "24px", fontWeight: "900", margin: "16px 0 8px" }}>{selectedPost.title}</h2>
        <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.35)", marginBottom: "20px" }}>{selectedPost.author} · {new Date(selectedPost.created_at).toLocaleDateString("de-DE")}</div>
      ...
      <div>
        ...
        {showNewPost && (
          <div style={{ padding: "20px", background: "rgba(200,150,62,0.06)", border: "1px solid rgba(200,150,62,0.2)", borderRadius: "12px", marginBottom: "24px" }}>
            <input style={{ ...S.input, width: "100%", marginBottom: "10px", boxSizing: "border-box" }} placeholder="Titel" value={newTitle} onChange={e => setNewTitle(e.target.value)} />
            <input style={{ ...S.input, width: "100%", marginBottom: "10px", boxSizing: "border-box" }} placeholder="Bild-URL (optional)" value={newImageUrl} onChange={e => setNewImageUrl(e.target.value)} />
            <textarea style={{ ...S.input, width: "100%", minHeight: "200px", resize: "vertical", boxSizing: "border-box" }} placeholder="Inhalt..." value={newContent} onChange={e => setNewContent(e.target.value)} />
        ...
        {!blogLoading && blogPosts.length === 0 && <div style={{ textAlign: "center", color: "rgba(255,255,255,0.3)", padding: "40px" }}>Noch keine Beiträge.</div>}
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          {blogPosts.map(post => (
            <div key={post.id} style={{ padding: "20px", borderRadius: "12px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", cursor: "pointer", transition: "all 0.2s" }}
              ...
              onMouseEnter={e => e.currentTarget.style.background = "rgba(200,150,62,0.06)"}
              onMouseLeave={e => e.currentTarget.style.background = "rgba(255,255,255,0.03)"}>
              {post.image_url && <img src={post.image_url} style={{ width: "100%", borderRadius: "8px", marginBottom: "12px", maxHeight: "200px", objectFit: "cover" }} />}
              <div style={{ fontSize: "16px", fontWeight: "700", marginBottom: "6px" }}>{post.title}</div>
              <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.35)", marginBottom: "10px" }}>{post.author} · {new Date(post.created_at).toLocaleDateString("de-DE")}</div>
              <div style={{ fontSize: "13px", color: "rgba(255,255,255,0.5)", lineHeight: "1.6" }}>{post.content.substring(0, 150)}…</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div>
          {user ? (
            ...
            <button style={{ background: "rgba(200,150,62,0.15)", border: "1px solid rgba(200,150,62,0.3)", borderRadius: "8px", padding: "7px 14px", color: "#C8963E", cursor: "pointer", fontSize: "13px", fontWeight: "600" }}
              onClick={() => setShowAuth(true)}>Anmelden</button>
          )}
        </div>
      </div>
      {!isChatView && (
        <div style={S.nav}>
          {tabs.map(tab => (
            <button key={tab.id} style={S.navBtn(activeTab === tab.id)} onClick={() => setActiveTab(tab.id)}>{tab.label}</button>
          ))}
        </div>
      )}
      <div style={{ ...S.content, padding: isChatView ? "12px" : "20px" }}>
        ...
        {activeTab === "shop" && (
          <div>
            ...
            <iframe src="https://human-rights-hub-merch.myspreadshop.de" style={{ width: "100%", height: "800px", border: "none", borderRadius: "12px" }} title="Human Rights Hub Merch Shop" />
          </div>
        )}
